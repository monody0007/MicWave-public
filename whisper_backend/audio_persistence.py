...
anydoor7) can still persist raw audio. WhisperWave deployments leave this
disabled because the client already caches audio on the user machine.

Env (read once at construction):
  WHISPERWAVE_SERVER_AUDIO_CACHE_ENABLED   "1" to enable (default "0")
  WHISPERWAVE_SERVER_AUDIO_CACHE_DIR       target dir; default
                                       ~/Library/Application Support/EchoWave/whisper/recent_audio
  WHISPERWAVE_SERVER_AUDIO_FILENAME_PREFIX prefix to mark the originating service
                                       (e.g. "anydoor7_" → distinguishes from
                                       IME-cached files at a glance in `ls`)
  WHISPERWAVE_SERVER_AUDIO_CACHE_LIMIT     queue maxsize hint, 0 = unlimited
...
_DEFAULT_DIR = "~/Library/Application Support/EchoWave/whisper/recent_audio"
_DEFAULT_MAX_BUFFER_BYTES = 28_800_000
...
class TurnAudioCache:
    ...
    def from_env(
        ...
        *,
        ...
        enabled = os.getenv("WHISPERWAVE_SERVER_AUDIO_CACHE_ENABLED", "0") == "1"
        cache_dir = os.getenv("WHISPERWAVE_SERVER_AUDIO_CACHE_DIR", _DEFAULT_DIR)
        prefix = os.getenv("WHISPERWAVE_SERVER_AUDIO_FILENAME_PREFIX", "")
        limit_raw = os.getenv("WHISPERWAVE_SERVER_AUDIO_CACHE_LIMIT", "0")
        try:
            limit = max(0, int(limit_raw))
        except ValueError:
            logger.warning(
                "Invalid WHISPERWAVE_SERVER_AUDIO_CACHE_LIMIT=%r, falling back to 0",
                limit_raw,
            )
            limit = 0
        return cls(
    ...
    def __init__(
        ...
        self._worker = threading.Thread(
            ...
            daemon=True,
            name=f"whisperwave-audio-cache-{filename_prefix or 'default'}",
        )
        self._worker.start()
        logger.info(
            "[audio_cache] enabled dir=%s prefix=%r limit=%d max_buffer_bytes=%d",
            self._cache_dir,
    ...
    def accumulate(self, pcm_bytes: bytes) -> None:
        if not self._enabled or not pcm_bytes or self._turn_discarded:
            return
        if len(self._buffer) + len(pcm_bytes) > self._max_buffer_bytes:
            self._buffer.clear()
            self._turn_discarded = True
            logger.warning(
                "[audio_cache] server-cache sidecar exceeded cap and was dropped "
                "for turn %s (max_buffer_bytes=%d); transcription continues",
