...
from ime_menubar import BrainwaveIMECore, Config, IMEState
...
def make_core(monkeypatch, instance_plans, *, events=None):
    ...
    core._begin_turn_spool = lambda: None
    core._cleanup_turn_spool_calls = 0

    def cleanup_turn_spool():
        core._cleanup_turn_spool_calls += 1

    core._cleanup_turn_spool = cleanup_turn_spool
...
async def test_audio_open_two_failures_returns_cleanly_to_idle(
    ...
    try:
        ...
        assert core._cleanup_turn_spool_calls == 1
...
def test_cleanup_timeout_does_not_wait_again_for_executor(capsys):
    ...
    core.loop = None
    core._cleanup_turn_spool = lambda: None
