...
class StreamingHomonymCorrector:
    ...
    def reset(self) -> None:
        # Drop the held tail without emitting. Used when the consumer is
        # about to overwrite the entire stream (e.g. fallback emit with
        # isNewResponse=True), so a later flush() can't append leftover
        # characters to the just-replaced text.
        self._buffer = ""
